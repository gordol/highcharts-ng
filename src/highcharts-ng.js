...
(function () {
    ...
    function highchart(highchartsNGUtils, $timeout) {
        ...
        var updatePlotLine = function(axis, newModelAxis, oldModelAxis) {
            if (angular.isDefined(newModelAxis.plotLines)) {
                for (var i = 0; i < newModelAxis.plotLines.length; i++) {
                    ensureIds(newModelAxis.plotLines, "plotlines-" + i + "-");
                }
            }
            if (angular.isDefined(oldModelAxis.plotLines)) {
                for (var l = 0; l < oldModelAxis.plotLines.length; l++) {
                    axis.removePlotLine(oldModelAxis.plotLines[l].id);
                }
            }
            if (angular.isDefined(newModelAxis.plotLines)) {
                for (var n = 0; n < newModelAxis.plotLines.length; n++) {
                    axis.addPlotLine(newModelAxis.plotLines[n]);
                }
            }
        };
